...
export default class HTTPSDirectorServer {
    constructor(options) {
        ...
        this.letsEncrypt = new LetsEncrypt(this.route53);
    }
    ...
    addRoutes(app) {
        app.post('/api/v1/certificate', asyncHandler(async (req, res) => {
            ...
            await this.route53.updateOurIPAddress(hostname, ip_address);
            let [cert, key] = await this.letsEncrypt.requestAndSaveCertificate(hostname);
            res.json({ cert, key });
        }));
    }
